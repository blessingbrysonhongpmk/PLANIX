 ...
 * Clean, organized, Linear-inspired sidebar architecture with 3 clear sections
 */
      ...
      <aside class="sidebar ${isMobileOpen ? 'mobile-open' : ''}">
        ...
        <div class="sidebar-header" style="display: flex; align-items: center; justify-content: space-between;">
          <a href="#" class="brand-logo" onclick="event.preventDefault(); window.store.setState({ currentView: 'dashboard', isMobileSidebarOpen: false })">
            <div class="brand-emblem" style="background: linear-gradient(135deg, #3B82F6 0%, #8B5CF6 100%); width: 34px; height: 34px; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; font-weight: 900; font-size: 18px;">P</div>
        ...
        <div class="sidebar-footer" style="padding: 12px 14px; border-top: 1px solid var(--border-subtle); display: flex; align-items: center; gap: 10px; background: rgba(255,255,255,0.02); border-radius: 12px; margin-top: 12px; cursor: pointer;" onclick="window.store.setState({ isSettingsModalOpen: true })">
          <div class="avatar" style="width: 34px; height: 34px; border-radius: 50%; background: linear-gradient(135deg, #3B82F6, #8B5CF6); display: flex; align-items: center; justify-content: center; font-weight: 700; color: white;">
          ...
          <div class="user-info" style="flex: 1; overflow: hidden;">
            ...
            <div style="font-size: 11px; color: #3B82F6; font-weight: 600;">Lvl ${state.user.level} • ${state.user.levelTitle}</div>
          </div>
  ...
  renderNavGroup(items, state) {
    return items.map(item => `
      <a class="nav-item ${state.currentView === item.id ? 'active' : ''}" 
        ...
        ${item.badge ? `<span class="nav-badge" style="margin-left: auto; background: rgba(59,130,246,0.2); color: #3B82F6; border: 1px solid rgba(59,130,246,0.3); border-radius: 99px; padding: 2px 7px; font-size: 11px; font-weight: 700;">${item.badge}</span>` : ''}
      </a>
    `).join('');
  }
}
