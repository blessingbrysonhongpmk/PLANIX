  ...
  render(state) {
    ...
    return `
      <div class="view-container animate-fade-in" style="padding-right: 10px;">
        ...
        <!-- ==========================================
             NATIVE MOBILE HABITS
             ========================================== -->
        <div class="mobile-only" style="padding-bottom: 80px;">
          ${habits.length === 0 ? `
            <div style="text-align: center; color: #8E8E9E; padding: 40px; font-size: 14px;">No habits tracked yet.</div>
          ` : `
            <div style="display: flex; flex-direction: column; gap: 12px;">
              ${habits.map(h => `
                <div style="background: #121217; border: 1px solid #22222A; border-radius: 16px; padding: 16px; display: flex; align-items: center; justify-content: space-between;" onclick="window.habitsView.toggleHabit('${h.id}')">
                  <div style="display: flex; align-items: center; gap: 12px;">
                    <div style="width: 48px; height: 48px; border-radius: 24px; border: 3px solid ${h.completedToday ? '#10B981' : '#2A2A32'}; background: ${h.completedToday ? 'rgba(16, 185, 129, 0.1)' : 'transparent'}; display: flex; align-items: center; justify-content: center; font-size: 20px; transition: all 0.2s ease;">
                      ${h.completedToday ? '✓' : '🔄'}
                    </div>
                    <div>
                      <div style="font-size: 15px; font-weight: 700; color: ${h.completedToday ? '#8E8E9E' : '#FFF'}; text-decoration: ${h.completedToday ? 'line-through' : 'none'};">${h.name}</div>
                      <div style="font-size: 11px; color: #8E8E9E; margin-top: 2px;">${h.category || 'General'}</div>
                    </div>
                  </div>
                  <div style="text-align: right;">
                    <div style="font-size: 16px; font-weight: 800; color: #F59E0B;">🔥 ${h.streak || 0}</div>
                    <div style="font-size: 10px; color: #8E8E9E; font-weight: 600; text-transform: uppercase;">Streak</div>
                  </div>
                </div>
              `).join('')}
            </div>
